...
void PollServer()
{
   ...
   if(action == "be") {
      MoveToBreakeven(broker_symbol, channel_id, signal_id);
      AckSignal(signal_id);
      return;
   }
   ...
   if(action == "close_now") {
      ClosePositions(broker_symbol, channel_id);
      ReportResult(signal_id, true, "Cerrado manualmente");
      AckSignal(signal_id);
      return;
   }
...
void MoveToBreakeven(string symbol, string channel_id, string signal_id)
{
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--) {
      ...
      double tick_size = SymbolInfoDouble(symbol, SYMBOL_TRADE_TICK_SIZE);
      if(tick_size <= 0) tick_size = 0.00001;

      if(MathAbs(PosInfo.StopLoss() - entry) < tick_size) continue; // ya está en BE

      if(Trade.PositionModify(PosInfo.Ticket(), entry, PosInfo.TakeProfit()))
         modified++;
      else
         Print("Error BE ticket=", PosInfo.Ticket(), " err=", GetLastError());
   }
   Print("BE procesado: ", modified, " posicion(es) de ", symbol);
   if(signal_id != "") {
      if(modified > 0)
         ReportResult(signal_id, true, "BE aplicado (" + IntegerToString(modified) + ")");
      else
         ReportResult(signal_id, false, "Sin posiciones para BE");
   }
}
